...
extension Font {
    static func ilDisplay(_ size: CGFloat = 28) -> Font {
        .custom(ILType.display, size: size, relativeTo: .largeTitle).weight(.bold)
    }
    static func ilTitle(_ size: CGFloat = 17) -> Font {
        .custom(ILType.title, size: size, relativeTo: .headline).weight(.bold)
    }
    static func ilBody(_ size: CGFloat = 13) -> Font {
        .custom(ILType.body, size: size, relativeTo: .body)
    }
    static func ilMono(_ size: CGFloat = 12) -> Font {
        .custom(ILType.mono, size: size, relativeTo: .caption).weight(.medium)
    }
}
